...
// pra abrir). Fica acima da zona dos toasts pra nunca ficar coberto por um toast passageiro.
function FAB({ onClick }) {
  return (
    <button
      onClick={onClick}
      className="fixed z-20 w-14 h-14 rounded-full flex items-center justify-center shadow-soft-lg no-print lg:hidden active:scale-95 transition-transform"
      style={{ right: '1.25rem', bottom: 'calc(5.5rem + env(safe-area-inset-bottom, 0px))', backgroundColor: 'var(--primary)' }}
...
function Header({ period, setPeriod, customRange, setCustomRange, search, setSearch, setSidebarOpen, insights }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center gap-2 sm:gap-3 px-3 sm:px-6 lg:px-8 py-3 sm:py-4 no-print" style={{ backgroundColor: 'var(--bg)', borderBottom: '1px solid var(--border)' }}>
      ...
      <div className={`sm:hidden ${searchExpanded ? 'flex-1 min-w-0' : 'shrink-0'}`}>
        {searchExpanded ? (
          <div className="relative w-full">
            ...
            <input
              autoFocus
              value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar lançamentos..."
              className="w-full pl-9 pr-9 py-2 rounded-xl text-base focus-ring" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--card)', color: 'var(--text)' }}
            />
            <button
          ...
          <button onClick={() => setSearchExpanded(true)} className="p-2.5 rounded-xl hover:bg-black/5" title="Buscar">
      ...
      <Card padding="p-4 sm:p-5">
        <div className="flex flex-wrap items-center gap-2 no-print">
          ...
          <div className="relative sm:hidden" ref={filtersRef}>
            ...
            {showFilters && (
              <div className="absolute left-0 mt-2 w-64 max-w-[calc(100vw-2rem)] rounded-xl shadow-soft-lg p-3 z-20 space-y-2.5" style={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)' }}>
...
export default function App() {
  ...
  return (
    <div className={`cerne-root flex h-screen overflow-hidden${rootClassSuffix}`} style={{ backgroundColor: 'var(--bg)', height: '100dvh' }}>
      ...
      {settings.fabEnabled !== false && <FAB onClick={() => setModal({ type: 'newTransaction' })} />}
