...
import {
  ...
  MessageCircle, Moon, Sun, Shirt, Bike, Building2, EyeOff, PersonStanding, Copy,
} from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      {showCalendarCardsRow && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {v('cardsPreview') && <div className={v('financialCalendar') ? '' : 'lg:col-span-2'}><CardsPreview cards={data.cards} transactions={data.transactions} accounts={data.accounts} onPayInvoice={actions.payCardInvoice} onAdvanceInstallments={actions.advanceAllFutureInstallments} onSeeAll={() => actions.goTo('cartoes')} /></div>}
        </div>
      )}
      {showPreviewRow && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
  ...
  return (
    <Card className="animate-fade-up" style={atRisk ? { border: '1px solid var(--expense)' } : undefined}>
      <div className="flex items-center justify-end gap-1 mb-2">
        <button onClick={() => setConfirmDelete(true)} className="p-1.5 rounded-lg hover:bg-black/5" title="Excluir"><Trash2 size={14} color="var(--text-soft)" /></button>
      ...
      {!editingBalance ? (
        <div className="flex items-center gap-1.5 mt-3">
          ...
          <button onClick={() => { setBalanceValue(acc.balance); setEditingBalance(true); }} className="p-1 rounded-lg hover:bg-black/5" title="Editar saldo"><Pencil size={12} color="var(--text-soft)" /></button>
        </div>
      ) : (
        <div className="flex items-center gap-2 mt-3">
      ...
      {isNegative ? (
        <p className="text-xs flex items-center gap-1 mt-1" style={{ color: 'var(--expense)' }}>
          <AlertCircle size={12} /> Conta negativa
        </p>
      ) : atRisk && (
        <p className="text-xs flex items-center gap-1 mt-1" style={{ color: 'var(--expense)' }}>
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-1">
          ...
          <p className="text-sm font-medium min-w-[150px] text-center" style={{ color: 'var(--text)' }}>{capitalizeFirst(monthLabel)}</p>
...
function CardsPage({ cards, transactions, accounts, recurring, settings, onAdd, onEdit, onDelete, onPayInvoice, onAdvanceInstallments, benefits, onAddBenefit, onDeleteBenefit, onUpdateBenefit, view = 'cartoes', onChangeView, onMarkPaid }) {
  const [showForm, setShowForm] = useState(false);
      ...
      {view === 'fatura' ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
            {cards.map((c, i) => (
              <CreditCardVisual key={c.id} card={c} transactions={transactions} accounts={accounts} onPayInvoice={onPayInvoice} onAdvanceInstallments={onAdvanceInstallments} onEdit={onEdit} onDelete={onDelete} gradient={CARD_GRADIENTS[i % CARD_GRADIENTS.length]} />
            ))}
          </div>
...
function RecurringExpensesPage({ recurring, accounts, cards, settings, onAdd, onDelete, onLaunchNow }) {
  const [showForm, setShowForm] = useState(false);
  const [confirmDelete, setConfirmDelete] = useState(null);
            ...
            <div style={{ width: '100%', height: 230 }}>
              <ResponsiveContainer>
                <BarChart data={chartData} layout="vertical" margin={{ left: 0, right: 20, top: 4, bottom: 4 }} barCategoryGap={10} onMouseLeave={() => setActiveBarIndex(null)}>
                  ...
                  <YAxis type="category" dataKey="name" tick={{ fontSize: 10, fill: 'var(--text-soft)' }} axisLine={false} tickLine={false} width={82} tickFormatter={(v) => (v.length > 10 ? `${v.slice(0, 9)}…` : v)} />
        ...
        <Card className={showChart ? 'lg:col-span-3' : 'lg:col-span-5'}>
          ...
          <div>
            {recurring.map((r, i) => {
              ...
              return (
                <div key={r.id} className="py-3 px-1" style={i > 0 ? { borderTop: '1px solid var(--border)' } : undefined}>
                  <div className="flex items-start gap-3">
                    <IconCircle icon={cat.icon} color={cat.color} soft={cat.soft} size={38} />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-sm font-medium break-words" style={{ color: 'var(--text)' }}>{r.name}</p>
                  ...
                  <div className="flex items-center gap-2 justify-end mt-2">
                    ...
                    <button onClick={() => setConfirmDelete(r)} className="p-1.5 rounded-lg hover:bg-black/5 shrink-0" title="Excluir"><Trash2 size={14} color="var(--text-soft)" /></button>
                  </div>
                </div>
              );
            })}
        ...
        <main className="flex-1 overflow-y-auto px-4 md:px-6 lg:px-8 py-6 print-area">
          {search.trim() ? (
            ...
            <>
              ...
              {activePage === 'cartoes' && <CardsPage cards={cards} transactions={transactions} accounts={accounts} recurring={recurring} settings={settings} onAdd={addCard} onEdit={editCard} onDelete={deleteCard} onPayInvoice={payCardInvoice} onAdvanceInstallments={advanceAllFutureInstallments} benefits={benefits} onAddBenefit={addBenefit} onDeleteBenefit={deleteBenefit} onUpdateBenefit={updateBenefit} view={cardsView} onChangeView={setCardsView} onMarkPaid={markTransactionPaid} />}
              ...
              {activePage === 'recorrentes' && <RecurringExpensesPage recurring={recurring} accounts={accounts} cards={cards} settings={settings} onAdd={addRecurring} onDelete={deleteRecurring} onLaunchNow={addRecurringAsTransaction} />}
