...
import {
  ...
  MessageCircle, Moon, Sun, Shirt, Bike, Building2, EyeOff, PersonStanding, Copy, Smartphone,
} from 'lucide-react';
...
const VISIBILITY_SCHEMA = [
  {
    ...
    items: [
      ...
      { key: 'accountsCardsSummary', label: 'Resumo por conta e cartão' },
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="flex items-center gap-2 px-3 sm:px-3.5 py-2 rounded-xl text-sm font-medium focus-ring"
...
function Header({ period, setPeriod, customRange, setCustomRange, search, setSearch, setSidebarOpen, insights }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center gap-2 sm:gap-3 px-3 sm:px-6 lg:px-8 py-3 sm:py-4 no-print" style={{ backgroundColor: 'var(--bg)', borderBottom: '1px solid var(--border)' }}>
      <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-xl hover:bg-black/5 shrink-0" title="Abrir menu">
      ...
      <div className="flex-1 min-w-0 flex justify-center px-1 sm:px-2">
        <div className="relative w-full max-w-sm min-w-0">
          ...
          <input
            ...
            className="w-full pl-9 pr-9 py-2 rounded-xl text-sm focus-ring" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--card)', color: 'var(--text)' }}
          />
          {search && (
            <button
              onClick={() => setSearch('')} title="Limpar busca"
              className="absolute right-2.5 top-1/2 -translate-y-1/2 p-0.5 rounded-full hover:bg-black/10"
            >
      ...
      <div className="relative shrink-0" ref={notifRef}>
        <button onClick={() => setNotifOpen(!notifOpen)} className="relative p-2.5 rounded-xl hover:bg-black/5 focus-ring" title="Notificações">
  ...
  return (
    <div className="mx-4 md:mx-6 lg:mx-8 mt-4 mb-2 no-print">
      <div className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ backgroundColor: 'var(--primary-soft)' }}>
        ...
        <button onClick={onDismiss} className="p-1 rounded-lg hover:bg-black/5 shrink-0" title="Dispensar"><X size={14} color="var(--primary-dark)" /></button>
      </div>
    </div>
  );
}
...
function KPIRow({ kpis, settings }) {
  const metaPercent = kpis.metaMensal.percent;
  const allCards = [
    { key: 'kpiSaldo', title: 'Saldo disponível', value: formatBRL(kpis.saldoDisponivel), description: 'Soma de todas as contas', icon: Wallet, color: 'var(--primary)', soft: 'var(--primary-soft)' },
            ...
            return (
              <div key={tx.id} className="group flex items-center gap-3 py-2 px-1 rounded-xl hover:bg-black/[0.02]">
                ...
                <div className="hidden sm:flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => onEdit(tx)} className="p-1.5 rounded-lg hover:bg-black/5" title="Editar"><Pencil size={13} color="var(--text-soft)" /></button>
                  <button onClick={() => onDelete(tx)} className="p-1.5 rounded-lg hover:bg-black/5" title="Excluir"><Trash2 size={13} color="var(--expense)" /></button>
                </div>
              </div>
            );
          })}
  ...
  return (
    <Card ref={cardRef} className="animate-fade-up" padding="p-5">
      <div className="flex items-start gap-3 mb-4">
        ...
        {!compact && (
          <button onClick={() => setConfirmDelete(true)} className="p-1 rounded-lg hover:bg-black/5 shrink-0" title="Excluir"><Trash2 size={14} color="var(--text-soft)" /></button>
        )}
      </div>
...
function CreditCardVisual({ card, transactions, accounts = [], gradient, onPayInvoice, onAdvanceInstallments, onEdit, onDelete }) {
  ...
  return (
    <div className="rounded-2xl p-5 text-white shadow-soft-lg" style={{ background: gradient }}>
      <div className="flex items-start justify-between mb-6">
        ...
        {onEdit && (
          <button onClick={() => setShowEditForm(true)} className="p-1.5 rounded-lg hover:bg-white/10" title="Editar cartão">
...
/* ---------- Resumo por conta e cartão (dashboard) ---------- */

function AccountsCardsSummary({ accounts, cards, transactions, onSeeAccounts, onSeeCards }) {
  if (accounts.length === 0 && cards.length === 0) return null;
  return (
    <Card className="animate-fade-up" padding="p-5 sm:p-6">
      <SectionTitle subtitle="saldo de cada conta e fatura atual de cada cartão">Resumo por conta e cartão</SectionTitle>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {accounts.map((a) => {
          const Icon = ACCOUNTS_ICONS[a.type] || Wallet;
          return (
            <button key={a.id} onClick={onSeeAccounts} className="flex items-center gap-3 p-3 rounded-xl text-left hover:bg-black/[0.02]" style={{ backgroundColor: 'var(--bg)' }}>
              <IconCircle icon={Icon} color="var(--invest)" soft="var(--invest-soft)" size={34} />
              <div className="min-w-0 flex-1">
                <p className="text-xs truncate" style={{ color: 'var(--text-soft)' }}>{a.bank}</p>
                <p className="text-sm font-semibold tabular-nums truncate" style={{ color: a.balance < 0 ? 'var(--expense)' : 'var(--text)' }}>{formatBRL(a.balance)}</p>
              </div>
            </button>
          );
        })}
        {cards.map((c) => {
          const invoice = computeCardInvoice(c, transactions);
          return (
            <button key={c.id} onClick={onSeeCards} className="flex items-center gap-3 p-3 rounded-xl text-left hover:bg-black/[0.02]" style={{ backgroundColor: 'var(--bg)' }}>
              <IconCircle icon={CreditCard} color="var(--primary)" soft="var(--primary-soft)" size={34} />
              <div className="min-w-0 flex-1">
                <p className="text-xs truncate" style={{ color: 'var(--text-soft)' }}>{c.bank} · fatura atual</p>
                <p className="text-sm font-semibold tabular-nums truncate" style={{ color: 'var(--text)' }}>{formatBRL(invoice)}</p>
              </div>
            </button>
          );
        })}
...
function DashboardPage({ data, actions }) {
  ...
  const allBlocks = ['kpiSaldo', 'kpiReceitas', 'kpiDespesas', 'kpiEconomia', 'kpiMeta', 'kpiPatrimonio', 'evolutionChart', 'categoryDonut', 'recentTransactions', 'monthSummary', 'goalsSection', 'financialCalendar', 'cardsPreview', 'recurringPreview', 'investmentsPreview', 'accountsCardsSummary', 'insights'];
  ...
  return (
    <div className="space-y-6">
      {showKPIRow && <KPIRow kpis={kpis} settings={settings} />}
      {showChartsRow && (
      ...
      {v('accountsCardsSummary') && <AccountsCardsSummary accounts={data.accounts} cards={data.cards} transactions={data.transactions} onSeeAccounts={() => actions.goTo('contas')} onSeeCards={() => actions.goTo('cartoes')} />}
    </div>
  );
}
                  ...
                  <tr key={tx.id} className="text-sm hover:bg-black/[0.02]">
                    ...
                    <td className="py-3 pr-1 no-print">
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => setEditing(tx)} className="p-1.5 rounded-lg hover:bg-black/5" title="Editar"><Pencil size={14} color="var(--text-soft)" /></button>
                        <button onClick={() => onDelete(tx)} className="p-1.5 rounded-lg hover:bg-black/5" title="Excluir"><Trash2 size={14} color="var(--expense)" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
  ...
  return (
    <Card padding="p-4">
      <div className="flex items-start gap-3 mb-3">
        ...
        <button onClick={() => setConfirmDelete(true)} className="p-1 rounded-lg hover:bg-black/5" title="Excluir"><Trash2 size={13} color="var(--text-soft)" /></button>
      </div>
  ...
  return (
    <Card className="animate-fade-up" style={atRisk ? { border: '1px solid var(--expense)' } : undefined}>
      <div className="flex items-start gap-3 mb-3">
        ...
        <button onClick={() => setConfirmDelete(true)} className="p-1.5 rounded-lg hover:bg-black/5 shrink-0" title="Excluir"><Trash2 size={14} color="var(--text-soft)" /></button>
      </div>
      {!editingBalance ? (
        <div className="flex items-center gap-1.5 mt-3 min-w-0">
          ...
          <button onClick={() => { setBalanceValue(acc.balance); setEditingBalance(true); }} className="p-1 rounded-lg hover:bg-black/5 shrink-0" title="Editar saldo"><Pencil size={12} color="var(--text-soft)" /></button>
        </div>
      ) : (
  ...
  return (
    <Card padding="p-4">
      <div className="flex items-start gap-3 mb-3">
        ...
        <button onClick={() => onDelete(benefit)} className="p-1 rounded-lg hover:bg-black/5" title="Excluir"><Trash2 size={13} color="var(--text-soft)" /></button>
      </div>
...
function MonthlyInvoicePage({ cards, transactions, accounts, cardGradients, onPayInvoice, onAdvanceInstallments, onMarkPaid }) {
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {visibleCards.map((c, i) => (
              <CreditCardVisual key={c.id} card={c} transactions={transactions} accounts={accounts} onPayInvoice={onPayInvoice} onAdvanceInstallments={onAdvanceInstallments} gradient={cardGradients[i % cardGradients.length]} />
            ))}
          </div>
...
function CardsPage({ cards, transactions, accounts, recurring, settings, cardGradients, onAdd, onEdit, onDelete, onPayInvoice, onAdvanceInstallments, benefits, onAddBenefit, onDeleteBenefit, onUpdateBenefit, view = 'cartoes', onChangeView, onMarkPaid }) {
  const [showForm, setShowForm] = useState(false);
  ...
  return (
    <div className="space-y-6">
      ...
      {view === 'fatura' ? (
        <MonthlyInvoicePage cards={cards} transactions={transactions} accounts={accounts} cardGradients={cardGradients} onPayInvoice={onPayInvoice} onAdvanceInstallments={onAdvanceInstallments} onMarkPaid={onMarkPaid} />
      ) : (
        <>
                  ...
                  return (
                    <tr key={inv.id} className="text-sm">
                      ...
                      <td className="py-3 pr-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => setEditing(inv)} className="p-1.5 rounded-lg hover:bg-black/5" title="Editar"><Pencil size={13} color="var(--text-soft)" /></button>
                          <button onClick={() => setConfirmDelete(inv)} className="p-1.5 rounded-lg hover:bg-black/5" title="Excluir"><Trash2 size={13} color="var(--expense)" /></button>
                        </div>
                      </td>
                    </tr>
                  );
...
function RecurringForm({ accounts = [], cards = [], initial, onSave, onClose, onLaunchNow }) {
  const [form, setForm] = useState(() => initial
    ? { ...initial }
    ...
    <div className="space-y-6">
        ...
        {showChart && (
          <Card className="lg:col-span-2">
            ...
            <div style={{ width: '100%', height: 230 }}>
              <ResponsiveContainer>
                <BarChart data={chartData} layout="vertical" margin={{ left: 0, right: 20, top: 4, bottom: 4 }} barCategoryGap={10} onMouseLeave={() => setActiveBarIndex(null)}>
                  <XAxis type="number" tick={{ fontSize: 10, fill: 'var(--text-soft)' }} axisLine={false} tickLine={false} tickFormatter={(v) => `R$${v}`} />
                  <YAxis type="category" dataKey="name" tick={{ fontSize: 10, fill: 'var(--text-soft)' }} axisLine={false} tickLine={false} width={96} tickFormatter={(v) => (v.length > 14 ? `${v.slice(0, 13)}…` : v)} />
                  <Bar dataKey="value" radius={[0, 6, 6, 0]} label={{ position: 'right', fontSize: 10, fill: 'var(--text-soft)', formatter: (v) => formatBRL(v) }}
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <span
                      ...
                      className="p-1.5 rounded-lg hover:bg-black/5 shrink-0" title="Excluir"
                    >
                      <Trash2 size={14} color="var(--text-soft)" />
                    </span>
                  </div>
      ...
      {confirmDelete && (
        <ConfirmModal
          ...
          description={`Tem certeza que deseja excluir "${confirmDelete.name}" da lista? Os lançamentos futuros dela também serão removidos — os que já venceram continuam no seu histórico. Essa ação não pode ser desfeita.`}
          onConfirm={() => { onDelete(confirmDelete); setConfirmDelete(null); }}
          onClose={() => setConfirmDelete(null)}
        />
      )}
    ...
    <Card>
      ...
      <div className="space-y-2">
        {VISIBILITY_SCHEMA.map((page) => {
          const isOpen = openCategory === page.key;
          return (
            <div key={page.key} className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
              <button
                onClick={() => setOpenCategory(isOpen ? null : page.key)}
                className="w-full flex items-center justify-between gap-2 px-4 py-3 focus-ring"
              >
                <span className="text-sm font-medium" style={{ color: 'var(--text)' }}>{page.label}</span>
                {isOpen ? <ChevronUp size={16} color="var(--text-soft)" /> : <ChevronDown size={16} color="var(--text-soft)" />}
              </button>
              {isOpen && (
                <div className="px-4 pb-4 space-y-3" style={{ borderTop: '1px solid var(--border)' }}>
                  <div className="flex items-center justify-between gap-3 pt-3">
                    <p className="text-sm flex items-center gap-1.5" style={{ color: 'var(--text-soft)' }}>{page.essentialLabel}</p>
                  ...
                  {page.items.map((item) => (
                    <div key={item.key} className="flex items-center justify-between gap-3">
                      ...
                      <ToggleSwitch checked={isVisible(settings, page.key, item.key)} onChange={(v) => toggleItem(page.key, item.key, v)} />
                    </div>
                  ))}
                </div>
              )}
